...
def test_model(_print, cfg, model, test_loader, tta=False):
    ...
    ids = []
    with torch.no_grad():
        for _, (image, _id) in enumerate(tbar):
            
            image = image.cuda()      
            image_lr = torch.flip(image, [-1])
            # output = model(image).view(-1, cfg.TRAIN.NUM_CLASSES)
            output = (torch.sigmoid(model(image)) + torch.sigmoid(model(image_lr))) / 2.0
    ...
    if not os.path.isdir(cfg.DIRS.SUB):
        os.mkdir(cfg.DIRS.SUB)
   
    np.save(os.path.join(cfg.DIRS.SUB, cfg.EXP + f"_fold{cfg.DATA.FOLD}.npy"), preds[:,1])
...
def train_loop(_print, cfg, model, train_loader,valid_loader, criterion, valid_criterion, optimizer, scheduler, start_epoch, best_metric):
    ...
    for epoch in range(start_epoch, cfg.TRAIN.EPOCHS):
        ...
        VALID
        """
        top_auc = valid_model(_print, cfg, model, valid_criterion, valid_loader, tta = cfg.INFER.TTA)
        is_best = top_auc > best_metric
        best_metric = max(top_auc, best_metric)
        _print("Current best: %.3f" % best_metric)
        #time
        time_ep = time.time() - time_ep
        _print(f"Time per epoch: {time_ep:.4f} seconds")

        #tensorboard
        if cfg.DEBUG == False:
            tb.add_scalars('Valid',
                            {'top_auc':top_auc}, epoch)
            save_checkpoint({
                "epoch": epoch + 1,
                "arch": cfg.EXP,
    ...
    _print(f"Time all: {time_all:.4f} mins")
